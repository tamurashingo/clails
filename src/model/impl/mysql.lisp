...
(defmethod create-connection-pool-impl ((database-type <database-type-mysql>))
  ...
  (let* ((config (getf *database-config* *project-environment*))
    ...
    (apply #'dbi-cp:make-dbi-connection-pool :mysql args)))
...
(defmethod to-string-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :string) value))

(defmethod to-text-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :text) value))

(defmethod to-integer-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :integer) value))

(defmethod to-float-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :float) value))

(defmethod to-decimal-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :decimal) value))

(defmethod to-datetime-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :datetime) value))

(defmethod to-date-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :date) value))

(defmethod to-time-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
  (funcall (get-cl-db-fn-by-type *mysql-type-convert-functions* :time) value))

(defmethod to-boolean-impl ((database-type <database-type-mysql>) value)
  (declare (ignore database-type))
