(in-package #:cl-user)
(defpackage #:clails-test/model/native-query/postgresql
  (:use #:cl
        #:rove
        #:clails/model/query)
...
(defpackage #:clails-test/model/native-query-db-postgresql
  (:use #:cl)
...
(in-package #:clails-test/model/native-query/postgresql)
...
(setup
  (setf *database-type* (make-instance 'clails/environment::<database-type-postgresql>))
  ...
  (clails/model/connection::with-db-connection-direct (conn)
    ...
    (dbi:do-sql conn
      "insert into posts (user_id, title, status, created_at, updated_at) values (?, ?, ?, now(), now())"
...
(select
 ("select id, name, created_at::text as created_text, id::text as id_text from users where id = :user_id::integer and name like :name_pattern::text")
 (defsql find-user-with-cast (user_id name_pattern)))

(deftest test-postgresql-type-casting
  (let ((result (execute-query find-user-with-cast '(:user_id "1" :name_pattern "Alice"))))
    (ok (= 1 (length result)))
    (ok (= 1 (getf (first result) :|id|)))
    (ok (string= "Alice" (getf (first result) :|name|)))
    (ok (stringp (getf (first result) :|created_text|)))
    (ok (string= "1" (getf (first result) :|id_text|)))))

;; PostgreSQL type casting in complex query
(select
 ("select p.id, p.user_id::text as user_id_text, p.title, count(*)::integer as post_count from posts p where p.user_id = :user_id::integer group by p.id, p.user_id, p.title having count(*) >= :min_count::integer order by p.id")
 (defsql find-posts-with-cast (user_id min_count)))

(deftest test-postgresql-type-casting-complex
  (let ((result (execute-query find-posts-with-cast '(:user_id "1" :min_count "1"))))
    (ok (>= (length result) 1))
    (ok (stringp (getf (first result) :|user_id_text|)))
    (ok (string= "1" (getf (first result) :|user_id_text|)))
    (ok (integerp (getf (first result) :|post_count|)))))

;; PostgreSQL array operations
(deftest test-postgresql-array-operations
  (clails/model/connection::with-db-connection-direct (conn)
    (dbi:do-sql conn
      "create table if not exists tags (id serial primary key, post_id integer, tag_names text[], created_at timestamp, updated_at timestamp)")
    (dbi:do-sql conn
      "insert into tags (post_id, tag_names, created_at, updated_at) values (?, ?, now(), now())"
      (list 1 "{lisp,programming,functional}"))
    (dbi:do-sql conn
      "insert into tags (post_id, tag_names, created_at, updated_at) values (?, ?, now(), now())"
      (list 2 "{lisp,draft}"))
    (dbi:commit conn))
  ...
  (select
   ("select post_id, tag_names from tags where :tag = any(tag_names)")
   (defsql find-posts-by-tag (tag)))

  (let ((result (execute-query find-posts-by-tag '(:tag "lisp"))))
    (ok (= 2 (length result)))
    (ok (= 1 (getf (first result) :|post_id|))))

  (clails/model/connection::with-db-connection-direct (conn)
    (dbi:do-sql conn "drop table if exists tags")
    (dbi:commit conn)))
...
;; INSERT query
(cl-batis:update
 ("insert into users (name, email, created_at, updated_at) values (:name, :email, now(), now())")
...
;; UPDATE query
(cl-batis:update
 ("update posts set title = :title, updated_at = now() where id = :id")
