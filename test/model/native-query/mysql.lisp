(in-package #:cl-user)
(defpackage #:clails-test/model/native-query/mysql
  (:use #:cl
        #:rove
        #:clails/model/query)
...
(defpackage #:clails-test/model/native-query-db-mysql
  (:use #:cl)
...
(in-package #:clails-test/model/native-query/mysql)
...
(setup
  (setf *database-type* (make-instance 'clails/environment::<database-type-mysql>))
...
(deftest test-mysql-fulltext-search
  (clails/model/connection::with-db-connection-direct (conn)
    (dbi:do-sql conn
      "create table if not exists articles (id int auto_increment primary key, title varchar(255), content text, fulltext(title, content)) engine=InnoDB")
    (dbi:do-sql conn
      "insert into articles (title, content) values (?, ?)"
      (list "Common Lisp Programming" "Common Lisp is a powerful programming language"))
    (dbi:do-sql conn
      "insert into articles (title, content) values (?, ?)"
      (list "Ruby on Rails Tutorial" "Rails is a web framework for Ruby"))
    (dbi:do-sql conn
      "insert into articles (title, content) values (?, ?)"
      (list "Functional Programming" "Lisp is a functional programming language"))
    (dbi:commit conn))

  (select
   ("select id, title, match(title, content) against(:query in natural language mode) as relevance from articles where match(title, content) against(:query in natural language mode) order by relevance desc")
   (defsql search-articles (query)))

  (let ((result (execute-query search-articles '(:query "Lisp"))))
    (ok (>= (length result) 2))
    (ok (> (getf (first result) :|relevance|) 0)))

  (clails/model/connection::with-db-connection-direct (conn)
    (dbi:do-sql conn "drop table if exists articles")
    (dbi:commit conn)))
