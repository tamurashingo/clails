...
(setup
  ...
  (clails/model/connection::with-db-connection-direct (connection)
    ...
    (dbi-cp:do-sql connection "insert into todo (created_at, updated_at, title, done, done_at) values ('2024-01-01 00:00:02', '2024-01-02 13:00:00', 'merge pr', false, null)"))
...
(deftest test-select
  (testing "no condition"
    (let* ((result (select 'clails-test-model::<todo>))
           (1st (first result))
           (2nd (second result))
           (3rd (third result)))
      (ok (= 3 (length result)))
      ...
      (ok (= (encode-universal-time 0 0 13 2 1 2024) (ref 3rd :updated-at)))
      (ok (= 0 (ref 3rd :done)))
      (ok (null (ref 3rd :done-at)))))
         
  (testing "single condition"
    (let* ((result (select 'clails-test-model::<todo> :where '(= done 0)))
           (1st (first result))
           (2nd (second result)))
      (ok (= 2 (length result)))
  ...
  (testing "other condition"
    (let* ((result (select 'clails-test-model::<todo> :where `(<= created-at "2024-01-01 00:00:00")))
           (1st (first result)))
      (ok (= 1 (length result)))
